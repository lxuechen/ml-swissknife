...
def plot(p):
    utils.plot_wrapper(plots=[dict(x=np.arange(n), y=p.value[i], label=f"{i}") for i in range(N)])
...
def parta():
    ...
    cons += [p[i] >= 0.0 for i in range(N)]
    cons += [sum(p[i]) == 1.0 for i in range(N)]

    obj = cp.Minimize(sum([cp.norm2(_next - _this) ** 2 for _next, _this in zip(p[1:], p[:-1])]))
...
def partb():
    ...
    for _next, _this in zip(p[1:], p[:-1]):
        ...
        for ui, vi in zip(_next, _this):
            loss = loss - 2.0 * cp.geo_mean(cp.vstack([ui, vi]))
...
def partc():
    ...
    obj = cp.Minimize(sum([cp.norm_inf(_next - _this) for _next, _this in zip(pcum[1:], pcum[:-1])]))
    cp.Problem(objective=obj, constraints=cons).solve()
