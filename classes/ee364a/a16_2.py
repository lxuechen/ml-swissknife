...
def main():
    ...
    objective = cp.Minimize(gamma * h * sum(cp.norm2(f[k]) for k in range(1, K + 1)))
    constraints = [v[k + 1] == v[k] + h / m * f[k] - h * g * e3 for k in range(1, K + 1)]
    constraints += [p[k + 1] == p[k] + (h / 2) * (v[k] + v[k + 1]) for k in range(1, K + 1)]
    constraints += [
        v[1] == v0,
        p[1] == p0.squeeze(),
    ]
    constraints += [p[k][2] >= alpha * cp.norm2(p[k][:2]) for k in range(1, K + 1)]
    constraints += [cp.norm2(f[k]) <= Fmax for k in range(1, K + 1)]
    constraints += [v[K + 1] == 0, p[K + 1] == 0]
