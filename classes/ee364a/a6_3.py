...
def main():
    ...
    for obj_fn, tag in zip((lambda aa: cp.abs(aa), lambda aa: cp.square(aa)), ("l1", "l2")):
        obj = sum(obj_fn(err(-np.pi + i * np.pi / N)) for i in range(1, 2 * N + 1))
        obj = cp.Minimize(obj)
        constraints = []
        prob = cp.Problem(obj, constraints).solve()

        ts = [-np.pi + i * np.pi / N for i in range(1, 2 * N + 1)]
        ys = []
        fs = []
        for t in ts:
            yt = y(t)
