...
true_mixture_weights = [0.3, 0.5, 0.2]
N = 100
k = 3
...
def generate_samples(mixture_weights):
    ...
    samples = np.vstack(
        [[np.random.normal(3, 2), np.random.uniform(-1, 2), np.random.laplace(-2, 3)] for _ in range(N)]
    )
...
def evaluate_density(samples):
    ...
    densities = [[norm.pdf(x, 3, 2), uniform.pdf(x, -1, 3), laplace.pdf(x, -2, 3)] for x in samples]
